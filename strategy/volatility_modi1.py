...
class VolatilityModi1(CryptoStrategy):
    ...
    def execute_buy_strategy(self, target_tickers, remain_buy_list):
        ...
        for target_ticker in target_tickers:
            # index += 1
            index = int([i for i, elements in enumerate(self.filtered_tickers_info) if elements['ticker'] == target_ticker][0])
            ...
            target_price = self.filtered_tickers_info[index]['target_price']
            ...
            if float(balance) > 0.0:
                ...
                if target_price < current_price:
                    market_timing_ratio = self.filtered_tickers_info[index]['market_timing_ratio']
                    yesterday_diff = self.filtered_tickers_info[index]['yesterday_diff']
    ...
    def execute_turn_end_process(self):
        ...
        for balance in balances[:]:
            currency = balance['currency']
            # logger.debug(balance)
            if currency == 'KRW' or balance['avg_buy_price'] == '0':
                logger.debug(balance)
                continue
            else:
                # logger.debug(balance)
    ...
    def update_target_tickers(self, start_time, end_time):
        ...
        for ticker in tickers:
            # org_df, mod_df = get_custom_1days_ohlcv(self.quotation_api, ticker, today_str, 0, 30, self.is_backtest)
            org_df, mod_df = get_custom_1days_ohlcv(self.quotation_api, ticker, today_str, 0, 1, self.is_backtest)
        ...
        for ticker in tickers:
            org_df, mod_df = get_custom_1days_ohlcv(self.quotation_api, ticker, today_str, 0, self.noise_ratio_average_day, self.is_backtest)
